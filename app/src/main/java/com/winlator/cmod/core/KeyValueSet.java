    ...
    public Iterator<String[]> iterator() {
        ...
        return new Iterator<String[]>() {
            ...
            public boolean hasNext() {
                // Skip empty / malformed segments so bad wincomponents never crash the app.
                while (start[0] < data.length()) {
                    if (end[0] < start[0]) end[0] = data.length();
                    if (start[0] >= end[0]) {
                        start[0] = end[0] + 1;
                        if (start[0] >= data.length()) return false;
                        end[0] = data.indexOf(",", start[0]);
                        if (end[0] == -1) end[0] = data.length();
                        continue;
                    }
                    int eq = data.indexOf("=", start[0]);
                    if (eq < start[0] || eq >= end[0]) {
                        start[0] = end[0] + 1;
                        end[0] = data.indexOf(",", start[0]);
                        if (end[0] == -1) end[0] = data.length();
                        continue;
