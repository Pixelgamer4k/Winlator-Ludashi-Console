...
public class Shortcut {
    ...
    public Shortcut(Container container, File file) {
        ...
        this.path = extractWineTargetPath(execArgs);
        ...
        try {
            ...
            Log.w("Shortcut", "Ignoring bad extraData while loading " + name, e);
        }
    }

    /**
     * Pull the executable path out of an Exec= line.
     * Must not confuse WINEPREFIX=".../.wine" with the wine command, and must
     * keep quoted paths that contain spaces (e.g. Hollow Knight Silksong.exe).
     */
    private static String extractWineTargetPath(String execArgs) {
        if (execArgs == null || execArgs.isEmpty()) return "";
        String lower = execArgs.toLowerCase(java.util.Locale.US);
        // Prefer " wine " so we skip ".../.wine" inside WINEPREFIX
        int idx = lower.lastIndexOf(" wine ");
        String rest;
        if (idx >= 0) {
            rest = execArgs.substring(idx + 6).trim();
        } else {
            idx = lower.lastIndexOf("wine ");
            if (idx < 0) return StringUtils.unescape(execArgs).replace("\"", "").trim();
            rest = execArgs.substring(idx + 5).trim();
        }
        if (rest.startsWith("\"")) {
            int end = rest.indexOf('"', 1);
            if (end > 1) return rest.substring(1, end).trim();
        }
        return StringUtils.unescape(rest).replace("\"", "").trim();
    }
