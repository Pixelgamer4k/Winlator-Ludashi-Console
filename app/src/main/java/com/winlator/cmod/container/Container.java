...
public class Container {
    ...
    // Builtin VC++2010 CRT: native MSVCP100/MSVCR100 often AV under FEX WoW64 (e.g. Far Cry 3).
    public static final String DEFAULT_ENV_VARS = "WINEDLLOVERRIDES=msvcp100,msvcr100=builtin WRAPPER_MAX_IMAGE_COUNT=0 VKD3D_SHADER_MODEL=6_6 ZINK_DESCRIPTORS=lazy ZINK_DEBUG=compact MESA_SHADER_CACHE_DISABLE=false MESA_SHADER_CACHE_MAX_SIZE=512MB mesa_glthread=true WINEESYNC=1 TU_DEBUG=noconform,sysmem DXVK_DISABLE_TIMELINE_SEMAPHORES=1";
    /** Legacy default that forced an on-screen DXVK/FPS overlay; stripped unless the user enables HUD. */
    public static final String LEGACY_DEFAULT_DXVK_HUD = "devinfo,version,gpuload,fps";
    public static final String DEFAULT_SCREEN_SIZE = "540x360";
    ...
    public static final String DEFAULT_WINCOMPONENTS = "direct3d=1,directsound=0,directmusic=0,directshow=0,directplay=0,xaudio=0,vcrun2010=0";
    public static final String FALLBACK_WINCOMPONENTS = "direct3d=1,directsound=1,directmusic=1,directshow=1,directplay=1,xaudio=1,vcrun2010=0";
    ...
    /**
     * Remove the old factory DXVK_HUD overlay when HUD/FPS is not enabled.
     * Custom DXVK_HUD values (user-configured) are kept.
     */
    public boolean stripLegacyDefaultDxvkHudIfNeeded() {
        if (isHudOverlayEnabled()) return false;
        String next = stripLegacyDefaultDxvkHud(envVars);
        if (next.equals(envVars)) return false;
        envVars = next;
        return true;
    }

    public boolean isHudOverlayEnabled() {
        String hudMode = getExtra("hudMode");
        if (hudMode != null && !hudMode.isEmpty()) {
            try {
                return Integer.parseInt(hudMode.trim()) != 0;
    ...
    public static String stripLegacyDefaultDxvkHud(String env) {
        if (env == null || env.isEmpty() || !env.contains("DXVK_HUD=")) return env != null ? env : "";
        ...
        for (String part : env.trim().split("\\s+")) {
            ...
            if (eq > 0) {
                ...
                if ("DXVK_HUD".equals(name) && (
                        value.equals(LEGACY_DEFAULT_DXVK_HUD) ||
                        value.equals("fps") ||
    ...
    public void loadData(JSONObject data) throws JSONException {
        ...
        if (stripLegacyDefaultDxvkHudIfNeeded()) {
            saveData();
        }
    }
    ...
    public static void checkObsoleteOrMissingProperties(JSONObject data) {
        try {
            ...
            if (!data.has("wincomponents")) return;
            String raw = data.optString("wincomponents", "");
            if (raw.isEmpty()) {
                data.put("wincomponents", DEFAULT_WINCOMPONENTS);
                return;
            }
            KeyValueSet wincomponents1 = new KeyValueSet(DEFAULT_WINCOMPONENTS);
            KeyValueSet wincomponents2 = new KeyValueSet(raw);
            String result = "";
            ...
            // Never let a bad wincomponents / extras string crash library load.
            try {
                if (data != null && !data.has("wincomponents")) {
                    data.put("wincomponents", DEFAULT_WINCOMPONENTS);
                }
